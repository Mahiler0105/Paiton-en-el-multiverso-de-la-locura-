...
export default class Paiton extends Phaser.Physics.Arcade.Sprite {
    ...
    sobrepiso(){ return this.paiton.body.onFloor() }
    ...
    update() {
        ...
        if (!this.killed) {
            ...
            if (!this.versus) {
                ...
                if (this.izquierda.isDown) { this.direccion = -1; this.moverse() }
                else if (this.derecha.isDown) { this.direccion = 1; this.moverse() }
                
                let izqder = (this.izquierda.isDown || this.derecha.isDown) && this.sobrepiso();
